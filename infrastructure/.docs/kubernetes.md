...
Sandboxes are created via `task env:create -- {task-id}` and destroyed via `task env:destroy -- {task-id}`.
...
See `.docs/standards/task-automation.md` for Taskfile conventions.
