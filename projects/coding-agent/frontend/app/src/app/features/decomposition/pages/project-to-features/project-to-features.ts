...
@Component({
  selector: 'app-project-to-features',
  standalone: true,
  imports: [
    CommonModule,
    MatIconModule,
  ...
  templateUrl: './project-to-features.html',
  styleUrl: './project-to-features.scss',
})
export class ProjectToFeaturesComponent implements OnInit {
  ...
  loadTasks(): void {
    ...
    this.decompositionService.listProjectTasks().subscribe({
      next: (tasks) => {
      ...
      error: (err: Error) => {
        ...
        this.error.set('Failed to load project tasks');
        this.loading.set(false);
      },
    });
  }
  ...
  openTask(task: TaskInfo): void {
    ...
    this.decompositionService.createTaskSession(task.id, 'project-to-features').subscribe({
      next: (session) => {
