...
export function createCorsConfig(corsOrigins?: string): NestJSCorsOptions {
  ...
  // Disable credentials if no origins or wildcard (can't use credentials with *)
  if (parsed.origins === false || parsed.origins === "*") {
 ...
 * Uses CODING_AGENT_CORS_ORIGINS for the coding agent backend
 * Defaults to allowing all origins for local development
 */
export function getCorsConfigFromEnv(): NestJSCorsOptions {
  const corsOrigins = process.env.CODING_AGENT_CORS_ORIGINS || "*";
