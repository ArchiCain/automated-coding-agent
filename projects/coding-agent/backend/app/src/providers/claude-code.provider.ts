...
export class ClaudeCodeProvider implements CodingAgentProvider {
  ...
  async *execute(request: AgentExecutionRequest): AsyncIterable<AgentMessage> {
    ...
    const queryOptions: Record<string, unknown> = {
      ...
      pathToClaudeCodeExecutable: process.env.CLAUDE_CODE_PATH || this.findClaudeCli(),
    };
  ...
  /**
   * Attempt to find the Claude CLI on the system.
   * Tries `which claude` first, then common installation paths.
   */
