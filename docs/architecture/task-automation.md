...
The template uses [Taskfile](https://taskfile.dev) to provide consistent automation across all services and environments:
...
For related automation concepts:
- **Environment Integration**: [Environment Configuration](../infrastructure/environment-configuration.md)
- **Development Workflows**: [Local Development](local-development.md)
- **Container Operations**: [Docker](../infrastructure/docker.md)
      - task: service-a:local:health
      - task: service-b:local:health
```

## Best Practices

### Task Naming
- Use consistent prefixes (`local:`, `dev:`, etc.)
- Descriptive action names (`start`, `stop`, `health`, `logs`)
- Hyphenated multi-word names (`health-check`, `full-restart`)
...
tasks:
  local:start:
    desc: "Start service with dependencies"
    deps:
      - local:build
    cmds:
      - docker compose up -d
