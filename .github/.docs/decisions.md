...
`OLLAMA_API_KEY`, `OPENCLAW_AUTH_TOKEN`, `GITHUB_APP_PRIVATE_KEY`,
and variables like `DEPLOY_HOST`, `GITHUB_APP_ID` — live in a single
root `.env` on the developer's laptop. Postgres credentials and the Keycloak client
